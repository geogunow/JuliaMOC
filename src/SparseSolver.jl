...
type SparseSolver
    ...
    # The fission matrix contains all fission terms
    _F::SparseMatrixCSC{Float64, Int64}
    
    # FIXME The fission matrix contains all fission terms
    ...
    # FIXME The fission matrix contains all fission terms
    _W_scale::SparseMatrixCSC{Float64, Int64}
    ...
    # FIXME A vector containing the previous iteration's scalar fluxes
    _const::Array{Float64}
...
function constructMatrices(self::SparseSolver)
    ...
    self._W_scale = spzeros(M, self._N)
    self._Ψ = ones(self._N)
    ...
    #TODO: track_id -> dir -> seg -> group -> polar
    #TODO: relations: page 44
    #TODO: avg ang flux i = (ang flux i - ang flux i-1) / seg_len_3D
                ...
                for s = 1:length(track._segments)
                    ...
                    for g = 1:num_groups
                        ...
                        for p = 1:self._num_polar_2
                            ...
                            #FIXME
                            self._W[i, j] -= weight * length_3D[p] / tau
                            if s != 1
                                j0 = j - self._num_polar_2 * num_groups
                                self._W[i, j0] += weight * length_3D[p] / tau
                            end
                            #=
                            self._W[i, j] += (weight * length_3D[p] * 
                                        (1.0/(1.0 - exp_val) - 1.0/tau))
                            if s != 1
                                j0 = j - self._num_polar_2 * num_groups
                                self._W[i, j0] += (weight * length_3D[p] *
                                             (1.0/tau - exp_val / 
                                              (1.0 - exp_val)))
                            end
                            =#
                            ...
                            self._W_scale[i,j] -= (weight * length_3D[p]
                                                   / (tau * vol))
                            if s != 1
                                j0 = j - self._num_polar_2 * num_groups
                                self._W_scale[i, j0] += (weight * length_3D[p] 
                                                   / (tau*vol))
                            end
                        end
                    end
                ...
                for s = 1:length(track._segments)
                    ...
                    for g = 1:num_groups
                        for p = 1:self._num_polar_2
                            ...
                            tau = sigma_t[g] * length_3D
                            exp_val = exp(-tau)
                            self._Ψ[i] = 1.0 / sigma_t[g] * (1.0 - exp_val)
                            if s != 1
                                prev = self._Ψ[i - self._num_polar_2 * num_groups]
                                self._Ψ[i] += prev * exp_val
                            end
                        end
                    end
                end
    ...
    self._T = T_tilde - H * S_tilde * self._W
...
function computeResidual(self::SparseSolver)
    ...
    #TODO: set old_pin_fission_rate to new_pin_fission_rate where zero
...
#FIXME: FUNCTION TO COMPUTE STARTING PSI if all sources are flat
...
function computeEigenvalue(self::SparseSolver)
    ...
    #FIXME
    num_groups = getNumEnergyGroups(self._track_generator._geometry)
    ...
    for iter = 1:self._max_iters
        ...
        #fission_source = self._F * self._Ψ
        fission_source = self._F_tilde * self._scalar_flux
        fission_source /= self._k_eff
        #fission_source += self._S * self._Ψ
        fission_source += self._S_tilde * self._scalar_flux
        fission_source /= (4 * pi)
        for r = 1:self._num_FSRs
            for g = 1:num_groups
                fission_source[(r-1)*num_groups+g] /= self._materials[r]._sigma_t[g]
            end
        end
        select_source = self._H * fission_source
        T2 = self._T + self._S
        self._Ψ = T2 \ select_source

        # Calculate the new scalar flux and eigenvalue
        #self._scalar_flux = self._W * self._Ψ # FIXME
        self._scalar_flux = self._W_scale * self._Ψ #+ self._const
        self._scalar_flux += fission_source * (4 * pi)
        computeKeff(self)
